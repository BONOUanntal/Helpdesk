...
async function openTicketById(ticketId: number) {
  // Recharge les tickets si nécessaire
...
<template>
  <div class="min-h-screen bg-slate-100 flex">
    ...
    <aside class="w-64 bg-slate-900 text-white flex flex-col p-6 gap-2">
      ...
      <button
        v-for="tab in ['tickets', 'clients', 'messages']"
        :key="tab"
        @click="activeTab = tab"
      ...
      >
        {{ tab === 'tickets' ? '🎫 Tickets' : tab === 'clients' ? '👥 Clients' : '💬 Messages' }}
      </button>
    ...
    <main class="flex-1 p-8">
      ...
      <div v-if="activeTab === 'tickets'">
        <h2 class="text-2xl font-bold mb-4">Tickets de mes applications</h2>
          ...
          <table class="w-full">
            <thead class="bg-slate-100">
              <tr>
                ...
                <th class="text-left px-6 py-4">Assigné à</th>
            ...
            <tbody>
              ...
              <tr v-if="tickets.length === 0">
                <td colspan="8" class="px-6 py-8 text-center text-slate-400">Aucun ticket</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow p-6 mb-6">
          <div class="grid grid-cols-2 gap-6">
            <div>
              ...
              <p class="font-semibold mt-1">{{ selectedTicket.subject }}</p>
            </div>
            <div>
              <p class="text-sm text-slate-500">Type de problème</p>
              <p class="font-semibold mt-1">{{ selectedTicket.issueType?.name ?? '—' }}</p>
            </div>
            <div>
              <p class="text-sm text-slate-500">Priorité</p>
              <p class="font-semibold mt-1">{{ selectedTicket.priority }}</p>
            </div>
            <div>
              <p class="text-sm text-slate-500">Application</p>
              <p class="font-semibold mt-1">{{ selectedTicket.application?.name ?? '—' }}</p>
            </div>
            <div>
              <p class="text-sm text-slate-500">Client</p>
              <p class="font-semibold mt-1">{{ selectedTicket.client?.name ?? selectedTicket.client?.email ?? '—' }}</p>
            </div>
            <div>
              ...
              <select
                ...
                class="border border-slate-300 rounded-lg px-3 py-2 text-sm mt-1 w-full"
              >
            ...
            <div>
              ...
              <div class="flex items-center gap-2 mt-1">
                <span class="font-semibold">{{ selectedTicket.assignedUser?.name ?? 'Non assigné' }}</span>
                <select
